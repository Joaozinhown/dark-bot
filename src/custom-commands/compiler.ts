...
import { parseCustomCommandDefinition } from './definition';
...
function localizations(value: LocalizedText): Record<string, string> {
  return { 'pt-BR': value.ptBR, 'en-US': value.enUS };
}

function compileParameter(option: ParameterOption): Record<string, unknown> {
  return {
...
function compileSubcommand(option: SubcommandOption): Record<string, unknown> {
  return {
...
function compileGroup(option: SubcommandGroupOption): Record<string, unknown> {
  return {
...
function compileOption(option: CommandOption): Record<string, unknown> {
  if (option.kind === 'parameter') return compileParameter(option);
  if (option.kind === 'subcommand') return compileSubcommand(option);
  return compileGroup(option);
}

function commandTextSize(payload: DiscordCommandPayload): number {
...
export function compileDiscordCommand(input: unknown): DiscordCommandPayload {
  const definition = parseCustomCommandDefinition(input);
...
function text(value: unknown, fallback: string): LocalizedText {
...
function fromParameter(raw: Record<string, unknown>, index: number): ParameterOption {
  const reverseTypes: Record<number, ParameterOption['type']> = {
...
function fromSubcommand(raw: Record<string, unknown>, index: number): SubcommandOption {
  ...
  return {
    ...
    name: text(raw.name_localizations, key),
    description: text(raw.description_localizations, readString(raw, 'description', key)),
    options: options.map((option, optionIndex) => fromParameter(option as Record<string, unknown>, optionIndex)),
  };
}

function fromOption(raw: Record<string, unknown>, index: number): CommandOption {
  if (Number(raw.type) === OPTION_TYPES.subcommand) return fromSubcommand(raw, index);
  if (Number(raw.type) === OPTION_TYPES.subcommand_group) {
    ...
    return {
      ...
      name: text(raw.name_localizations, key),
      description: text(raw.description_localizations, readString(raw, 'description', key)),
      options: options.map((option, optionIndex) => fromSubcommand(option as Record<string, unknown>, optionIndex)),
    };
  }
  return fromParameter(raw, index);
}

export function createNativeFactoryDefinition(
  ...
  return parseCustomCommandDefinition({
    schemaVersion: 1,
    ...
    command: {
      ...
      options: options.map((option, index) => fromOption(option as Record<string, unknown>, index)),
