...
exports.customCommandService = exports.CustomCommandError = exports.ARCHIVED_COMMAND_STATUSES = void 0;
const node_crypto_1 = require("node:crypto");
const client_1 = __importDefault(require("../database/client"));
const definition_1 = require("./definition");
const compiler_1 = require("./compiler");
exports.ARCHIVED_COMMAND_STATUSES = ['archived', 'factory_restored'];
...
exports.CustomCommandError = CustomCommandError;
function parseStoredDefinition(value) {
    try {
        return (0, definition_1.parseCustomCommandDefinition)(JSON.parse(value));
    }
    catch (error) {
        throw new CustomCommandError('INVALID_STORED_DEFINITION', `Definicao armazenada invalida: ${error instanceof Error ? error.message : String(error)}`, 500);
    }
}
...
function nativeFactoryItem(source) {
    const definition = (0, compiler_1.createNativeFactoryDefinition)(source.payload, source.requireBotAdmin);
...
function activeWhere(guildId) {
    return {
        guildId,
        status: { notIn: [...exports.ARCHIVED_COMMAND_STATUSES] },
    };
}
async function requireCommand(guildId, commandId) {
    const command = await client_1.default.customCommand.findFirst({
        where: { id: commandId, guildId },
        include: { versions: { orderBy: { version: 'desc' } } },
    });
    if (!command)
        throw new CustomCommandError('COMMAND_NOT_FOUND', 'Comando nao encontrado.', 404);
    return command;
}
...
async function validateNameAvailability(guildId, definition, stableKey, nativeCommands) {
    const rows = await client_1.default.customCommand.findMany({ where: activeWhere(guildId) });
    const overriddenFactories = new Set(rows.filter(row => row.sourceType === 'native').map(row => row.factoryCommandName));
    const reserved = new Map();
    for (const source of nativeCommands) {
        const name = String(source.payload.name ?? '');
        if (!name || overriddenFactories.has(name) || stableKey === `native:${name}`)
            continue;
        reserved.set(name.toLowerCase(), `/${name}`);
    }
    for (const row of rows) {
        if (row.stableKey === stableKey)
            continue;
        const existing = parseStoredDefinition(row.draftDefinition);
        for (const name of commandNames(existing))
            reserved.set(name, `/${existing.command.name.ptBR}`);
    }
...
exports.customCommandService = {
    async listCatalog(guildId, nativeCommands) {
        const rows = await client_1.default.customCommand.findMany({
            where: activeWhere(guildId),
            include: { versions: { orderBy: { version: 'desc' }, take: 20 } },
            orderBy: { atualizadoEm: 'desc' },
        });
        const overrides = new Map(rows.filter(row => row.sourceType === 'native').map(row => [row.factoryCommandName, row]));
    ...
    async get(guildId, commandId) {
        return rowToCatalog(await requireCommand(guildId, commandId));
    },
    async saveDraft(input) {
        const definition = (0, definition_1.parseCustomCommandDefinition)(input.definition);
        ...
        const stableKey = existing?.stableKey
            ?? (factoryName ? `native:${factoryName}` : `custom:${(0, node_crypto_1.randomUUID)()}`);
        ...
        const row = existing
            ? await client_1.default.customCommand.update({
                where: { id: existing.id },
                data,
                include: { versions: { orderBy: { version: 'desc' }, take: 20 } },
            })
            : await client_1.default.customCommand.create({
                data: { ...data, enabled: true, createdByUserId: input.actorUserId },
                include: { versions: true },
            });
        return rowToCatalog(row);
    },
    async publish(guildId, commandId, actorUserId) {
        const command = await requireCommand(guildId, commandId);
        const definition = parseStoredDefinition(command.draftDefinition);
        const activePublishedCount = await client_1.default.customCommand.count({
            where: { guildId, enabled: true, publishedVersionId: { not: null }, status: { notIn: [...exports.ARCHIVED_COMMAND_STATUSES] } },
        });
        ...
        const updated = await client_1.default.$transaction(async (transaction) => {
    ...
    async rollback(guildId, commandId, versionId, actorUserId) {
        const command = await requireCommand(guildId, commandId);
        const version = command.versions.find(item => item.id === versionId);
        if (!version)
            throw new CustomCommandError('VERSION_NOT_FOUND', 'Versao nao encontrada.', 404);
        await client_1.default.customCommand.update({
            where: { id: command.id },
    ...
    async setEnabled(guildId, commandId, enabled, actorUserId) {
        await requireCommand(guildId, commandId);
        const updated = await client_1.default.customCommand.update({
            where: { id: commandId },
    ...
    async archive(guildId, commandId, actorUserId) {
        const command = await requireCommand(guildId, commandId);
    ...
    async clone(sourceGuildId, commandId, targetGuildId, actorUserId, name, nativeCommands) {
    ...
    async markSyncError(commandId) {
        await client_1.default.customCommand.update({ where: { id: commandId }, data: { status: 'sync_error' } });
    },
    async setDiscordCommandId(commandId, discordCommandId) {
        await client_1.default.customCommand.update({ where: { id: commandId }, data: { discordCommandId } });
    },
    async listPublished(guildId) {
        const rows = await client_1.default.customCommand.findMany({
            where: {
                guildId,
                enabled: true,
    ...
    async listRegistrationState(guildId) {
        const rows = await client_1.default.customCommand.findMany({
            where: activeWhere(guildId),
            include: { versions: true },
        });
    ...
    async findPublishedByDiscordId(guildId, discordCommandId) {
        const command = await client_1.default.customCommand.findFirst({
            where: {
                guildId,
                discordCommandId,
    ...
    containsScript: definition_1.containsScript,
};
