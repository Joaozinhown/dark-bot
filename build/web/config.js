...
exports.readPanelConfig = readPanelConfig;
const zod_1 = require("zod");
const PRODUCTION_REDIRECT_URI = 'https://admin-dta-bot.discloud.app/api/auth/callback';
const enabledConfigSchema = zod_1.z.object({
    CLIENT_ID: zod_1.z.string().regex(/^\d{16,22}$/),
    DISCORD_CLIENT_SECRET: zod_1.z.string().min(1),
    DISCORD_REDIRECT_URI: zod_1.z.url(),
    PANEL_COOKIE_SECRET: zod_1.z.string().min(32),
    PANEL_ENCRYPTION_KEY: zod_1.z.string().min(1),
...
function readPanelConfig(environment = process.env) {
    if (environment.ADMIN_PANEL_ENABLED !== 'true')
